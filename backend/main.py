...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://hdfc-image-report.vercel.app/"],  # Your frontend
...
IGNORE the "Weight" column for segment. 
For ALL rows in this table, segment = "All GVW & PCV 3W, GCV 3W"
Because this is CV business. , FGI STATE is the location, FGI ZONE is the LOCATION so whenever writing in location do FGI STATE: FGI ZONE, OD out flow column is Policy Type and under it there are value which  is  payin and same for TP Outflow .

Note: P Car SATP means PVT CAR TP segment 
TW SATP means TW TP segment
sometimes in the data TW SATP Bike or TW SATP Scooter or other  is given , consider it as TW TP segment  only and write in the remark Bike and any other information.
TW COMP somes in the segment of TW SAOD + COMP 
sometimes in one segment 2 things are given : PO means PO for this and PO for that , so what you do is make ir two rows 

for example if in the input file  TW SATP Scooter : PO 35% for preferred and PO 25% for non preferred is given
so what you do is make two rows for 35 % as well as for 25 %
the above thing was just an example okay !!
